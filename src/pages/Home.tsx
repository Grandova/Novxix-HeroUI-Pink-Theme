...
import { usePortalTheme } from './portal/use-portal-theme'
import './portal/home-theme.css'
import {
...
function GlowCard({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div
      ...
      className={`vmnet-glow-card relative rounded-xl p-px bg-border/50 ${className ?? ''}`}
    >
      <div
        className="absolute inset-0 rounded-xl transition-opacity duration-500"
        style={{ opacity: 'var(--go,0)', background: 'radial-gradient(250px circle at var(--gx,0) var(--gy,0),color-mix(in srgb,var(--primary) 20%,transparent),transparent 40%)' }}
      />
...
function DashboardPreview() {
  ...
  const resources = [
    ...
    { label: '内存', value: 67, color: 'bg-rose-300' },
...
function FeatureSection({ icon: Icon, title, description, items, visual, reverse }: {
  ...
  return (
    <div className={`vmnet-home-feature grid lg:grid-cols-2 gap-10 lg:gap-16 items-center ${reverse ? 'lg:[direction:rtl] *:lg:[direction:ltr]' : ''}`}>
      <Reveal>
        <div>
...
function PerfVisual() {
  ...
  const bars = [
    ...
    { label: 'IOPS', value: 85, max: 100, unit: 'K', color: 'bg-rose-300', spaced: false },
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {banners[current]?.image ? (
            ...
            <div className="w-full h-full" style={{ background: "radial-gradient(ellipse at top, #82505e, #30262a 60%, #1c171a)" }} />
          )}
        </motion.div>
      </AnimatePresence>
...
export default function Home() {
  ...
  return (
    <>
      {/* ═══ Hero ═══ */}
      <section className={`vmnet-home-hero relative overflow-hidden ${hasBanners ? "min-h-[100svh]" : ""}`}>
        {hasBanners ? (
        ...
        <div className={`relative max-w-6xl mx-auto px-6 flex flex-col ${hasBanners ? "min-h-[100svh] justify-center pt-16" : "pt-16 sm:pt-24 pb-20 sm:pb-28"}`}>
          <div className="vmnet-home-intro text-center max-w-3xl mx-auto">
            <BlurIn>
            ...
            <BlurIn delay={0.3} className="vmnet-home-actions mt-8 sm:mt-10 flex items-center justify-center gap-3">
          ...
          {!hasBanners && (
            <BlurIn delay={0.5} className="vmnet-home-preview mt-14 sm:mt-20">
              <DashboardPreview />
            </BlurIn>
          )}
        </div>
      ...
      {/* ═══ 信任指标 ═══ */}
      {(cfg?.stats.length ?? 0) > 0 && <section className="vmnet-home-stats border-t border-dashed border-border/60 bg-muted/20">
      ...
      {/* ═══ 功能深度展示 ═══ */}
      {(cfg?.features.length ?? 0) > 0 && <section className="vmnet-home-features border-t border-dashed border-border/60">
      ...
      {/* ═══ 工作流程 ═══ */}
      {(cfg?.steps.length ?? 0) > 0 && <section className="vmnet-home-steps border-t border-dashed border-border/60 bg-muted/20">
        <div className="max-w-6xl mx-auto px-6 py-20 sm:py-28">
          <Reveal>
      ...
      {/* ═══ 数据中心 ═══ */}
      {regions.length > 0 && (
        <section className="vmnet-home-regions border-t border-dashed border-border/60">
          <div className="max-w-6xl mx-auto px-6 py-20 sm:py-28">
            <Reveal>
      ...
      {/* ═══ FAQ (CMS) ═══ */}
      {faqs.length > 0 && <section className="vmnet-home-faq border-t border-dashed border-border/60">
      ...
      {/* ═══ CTA ═══ */}
      {cfg?.cta.title && <section className="vmnet-home-cta border-t border-dashed border-border/60 bg-muted/20">
        <div className="max-w-6xl mx-auto px-6 py-20 sm:py-28">
          <Reveal>
            <div className="text-center">
