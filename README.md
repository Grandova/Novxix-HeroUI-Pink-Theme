...
面向 **Novaix v0.4.4** 的淡粉色主题。以奶白、暖灰与玫瑰粉搭配门户侧栏和响应式首页，保留原有业务入口与操作流程。
...
| 主题 ID | `hero-pink` |
| 当前版本 | `1.4.3` |
| Novaix 要求 | `~0.4.4`（`>=0.4.4` 且 `<0.5.0`） |
| 安装包 | `hero-pink.zip` |

本次更名也将主题 ID 从 `vmnet-theme` 改为 `hero-pink`。如果已安装旧版，面板会将 Hero Pink 识别为另一个主题；上传后选择并启用新主题即可。

版本约束不代表已验证后续所有补丁版本。本主题的 API 生成及构建基线为 Novaix **v0.4.4**；跨版本使用前应在测试面板验证。

## 1.4.3 更新

同步官方前端 v0.4.4（`6596fa8`），保留 Hero Pink 的首页、门户布局及配色。

- 用户实例详情新增自动续费开关，按官方规则对非按小时计费、且有到期时间的实例显示。
- 后台实例概览支持修改到期时间或设为永不过期，按站点时区编辑并提交 UTC 时间。
- 同步持续折扣、支付金额变更处理、流量显示刷新、IP 分配策略及节点上联网卡设置等官方更新。
- API 定义及最低版本要求更新为 Novaix v0.4.4。
...
需要 **Node.js 22.12+** 和 **pnpm 10**。仓库已包含 v0.4.4 的公开接口定义，构建不需要向你的面板索取文档或读取账号数据。
...
├── layouts/                       # 原有布局组件
└── api/                           # 自动生成，不提交
api-schema/novaix-v0.4.4.json          # 构建所需的公开 API 定义
...
本次更新的回归测试可在构建后执行 `pnpm exec playwright test`；首次运行需安装浏览器：`pnpm exec playwright install chromium`。本次生产构建、改动文件 ESLint 检查及 10 项浏览器回归检查已通过；测试以模拟接口检查自动续费、到期时间修改、订单金额变更和移动端界面。

此前布局版本已执行生产构建、门户 34 项回归检查，以及首页与门户的多尺寸检查；另外验证了购买表单、套餐金额更新、短窗口侧栏、个人资料键盘切换和主题作用域清理。

浏览器检查使用模拟接口数据，未代替真实面板上的支付、创建实例或其他端到端业务验证。本次功能回归脚本保留在 `e2e/`，临时截图及测试运行产物不进入版本控制。
...
`api-schema/novaix-v0.4.4.json` 提取自 Novaix v0.4.4 官方发行程序内嵌的公开 API 文档（下载包已与官方 SHA-256 校验值核对），用于重建 API 客户端，不包含本地面板账号、令牌或业务数据。
